...
Another approach for NDP in memory employs custom memory chips with resistive bitcells. In \cite{guo2011resistive} and \cite{guo2013ac}, a resistive associative memory is used as the main memory. Resistive bitcells are also used in analog computing \cite{boboila2012active}‎\cite{shafiee2016isaac} , proposing PiM structures that allow the memory array to be used as RAM, scratchpad and analog dot product engine. Both trends are still limited to memory NDP. 
In-storage NDP, an SSD comprising an array of Flash chips, is amended by an embedded CPU \cite{kang2013enabling} or GPU \cite{cho2013xsd} to perform some processing tasks in the SSD rather than in the host CPU, as demonstrated in Figure \ref{fig:in_vs_near_data}a. Often called Intelligent (or Smart) SSD, the main idea is increasing effective bandwidth to the in-SSD processors by connecting these processors directly to the flash banks inside the SSD. However, the storage bandwidth inside the SSD is still limited by the read/write ports of the flash devices. 
